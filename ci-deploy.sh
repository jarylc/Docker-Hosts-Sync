...
docker buildx build --push --cache-from=type=local,src=cache --platform "linux/amd64,linux/arm64,linux/arm/v7,linux/368,linux/arm/v6,linux/ppc64le,linux/s390x" -t "${REGISTRY_IMAGE}:${CI_COMMIT_SHORT_SHA}" -t "${REGISTRY_IMAGE}:${1}" .
